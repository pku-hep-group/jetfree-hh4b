...
These drive the real export entry point, ``train.onnx``, so the whole
...
from nn.model.ParticleTransformer import RMSNorm
...
def _make_args(workdir, version):
    """Minimal args namespace accepted by ``train.onnx`` / ``model_setup``."""
